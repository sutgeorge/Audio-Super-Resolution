...
axes[0].set_title("Low-res (1200 samples = 25 ms)")
axes[0].plot(batch_used_for_prediction[random_batch_index])
axes[1].set_title("High-res (4800 samples = 100 ms)")
axes[1].plot(high_res_chunks[random_batch_index])
axes[2].set_title("Super-res (4800 samples = 100 ms)")
axes[2].plot(result[random_batch_index])
axes[3].set_title("Interpolated (4800 samples = 100 ms)")
...
nrmse_high_res_super_res = normalised_root_mean_squared_error(high_res_chunk, super_res_chunk).numpy()
nrmse_high_res_interpolated = normalised_root_mean_squared_error(high_res_chunk, interpolated_chunk).numpy()
print(nrmse_high_res_super_res)
print(nrmse_high_res_interpolated)
figure.tight_layout()
figure.suptitle("Whole chunks (high-res/super-res NRMSE={:.4f}, high-res/interpolated NRMSE={:.4f})".format(nrmse_high_res_super_res, nrmse_high_res_interpolated))
...
nrmse_parts_of_the_chunk_high_res_interpolated = normalised_root_mean_squared_error(part_of_the_high_res_chunk, part_of_the_interpolated_chunk)
figure.tight_layout()
